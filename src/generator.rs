...
pub fn generate_via_csv(path: &str) -> std::io::Result<()> {
    ...
    for result in reader.records() {
        ...
        let params = FileGenerationParams {
            ...
            documentation: record.get(3).map(|s| s.to_string()),
            edition: record.get(3).map(|s| s.to_string()),
            email: record.get(3).map(|s| s.to_string()),
            homepage: record.get(3).map(|s| s.to_string()),
            ...
            readme: record.get(8).map(|s| s.to_string()),
